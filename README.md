...
**Important:** Prefer `step-security/setup-ruby@v1`.
...
* Use Bundler 2.2.18+ on Windows (older versions have [bugs](https://github.com/step-security/setup-ruby/issues/209#issuecomment-889064123)) by not setting the `bundler:` input and ensuring there is no `BUNDLED WITH 1.x.y` in a checked-in `Gemfile.lock`.
* The default shell on Windows is not Bash but [PowerShell](https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-syntax#using-a-specific-shell).
  This can lead issues such as multi-line scripts [not working as expected](https://github.com/step-security/setup-ruby/issues/13).
* The `PATH` contains [multiple compiler toolchains](https://github.com/step-security/setup-ruby/issues/19). Use `where.exe` to debug which tool is used.
